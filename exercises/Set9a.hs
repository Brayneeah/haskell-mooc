...
sumSuccess measurements
    | not (null successes) = Right $ sum successes
    | otherwise = Left "no data"
    where successes = foldl rights [] measurements
          rights acc (Right x) = x:acc
          rights acc _         = acc
...
instance Eq Text where
    (==) (Text s1@(x:xs)) (Text s2@(y:ys))
        | x == y = Text xs == Text ys
        | isSpace x = Text xs == Text s2
...
compose m1 m2 = foldl (\acc (a,b) -> maybe acc ((:acc) . (a,)) $ lookup b m2) [] m1
