...
countdown n = foldl (++) "Ready! " (reverse [show n' ++ "... " | n' <- [1..n]]) ++ "Liftoff!"
...
smallestDivisor n
    | even n = 2
    | otherwise = smallestDivisor' 3
    where smallestDivisor' i
            | n == i = n
            | n `mod` i == 0 = i
            | otherwise = smallestDivisor' $ i + 1
...
biggestPrimeAtMost n = snd $ fromMaybe (True,n) $ find fst primes
    where primes = reverse $ map (\n' -> (isPrime n', n')) [3..n]
