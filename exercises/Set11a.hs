...
readWords n = replicateM n getLine >>= return . sort
...
readUntil f = getLine >>= readUntil' []
    where readUntil' xs s
            | f s = return xs
            | otherwise = getLine >>= readUntil' (xs++[s])
...
isums :: Int -> IO Int
isums n = replicateM n readLn >>= foldM f 0
    where f total n' = do
