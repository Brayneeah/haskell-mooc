...
add e (Set s) = Set $ if e `notElem` s then sort $ e : s else s
...
data State = Start | Egged | Floured | Sugared | Mixable | Mixed | Error | Finished
    deriving (Eq,Show)
...
reverseNonEmpty (x:|xs) = x' :| xs'
    where (x':xs') = reverse (x:xs)
...
instance Semigroup Distance where
    (Distance m1) <> (Distance m2) = Distance $ m1 + m2

instance Semigroup Time where
    (Time m1) <> (Time m2) = Time $ m1 + m2

instance Semigroup Velocity where
    (Velocity m1) <> (Velocity m2) = Velocity $ m1 + m2
...
instance Ord a => Semigroup (Set a) where
    (Set s1) <> (Set s2) = Set $ sort $ union s1 s2
...
passwordAllowed s (MinimumLength len) = length s >= len
passwordAllowed s (ContainsSome s')   = any (`elem` s') s
passwordAllowed s (DoesNotContain s') = not $ any (`elem` s') s
...
evaluate (Operation op n1 n2) = ops op (evaluate n1)  (evaluate n2)
    where ops "+" = (+)
          ops "*" = (*)
          left = evaluate n1
          right = evaluate n2
...
render (Operation op l r) = "(" ++ render l ++ op ++ render r ++ ")"
