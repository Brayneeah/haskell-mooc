...
rgb (Invert c)  = zipWith (-) [1.0, 1.0, 1.0] (rgb c)
...
foldBin :: (Bin -> a -> a) -> a -> Bin -> a
foldBin f z End      = z
foldBin f z b@(O xs) = f b (foldBin f z xs)
foldBin f z b@(I xs) = foldBin f (f b z) xs
