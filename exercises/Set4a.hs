...
distinct = snd . foldl (\(acc, result) x -> (x:acc, result && notElem x acc)) ([], True)
...
longest :: Ord a => [[a]] -> [a]
longest xs = let longest_len = maximum $ map length xs
                 longest_lists = filter ((== longest_len) . length) xs
             in  minimumBy (\a b -> compare (head a) (head b)) longest_lists
