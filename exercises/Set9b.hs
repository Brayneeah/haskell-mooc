...
prettyPrint s coords = prettyPrint' (1,1)
    where
        prettyPrint' :: Coord -> String
        prettyPrint' cur@(x, y)
            | y > s = "\n" ++ prettyPrint' (nextRow cur)
            | x > s = ""
            | otherwise = (if cur `elem` coords
                           then "Q"
                           else ".") ++ prettyPrint' (nextCol cur)
...
danger candidate queens = or ([sameRow, sameCol, sameDiag, sameAntidiag] <*> [candidate] <*> queens)
...
prettyPrint2 s queens = prettyPrint' (1,1)
    where prettyPrint' cur@(x, y)
            | x > s = ""
            | y > s = "\n" ++ prettyPrint' (nextRow cur)
            | cur `elem` queens = "Q" ++ prettyPrint' (nextCol cur)
            | danger cur queens = "#" ++ prettyPrint' (nextCol cur)
            | otherwise = "." ++ prettyPrint' (nextCol cur)
